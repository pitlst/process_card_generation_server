"""工序卡输入数据模型 —— 对应 Excel 中 6 个 Sheet 的结构。"""
...
class ImageBatchUploadResponse(BaseModel):
    ...
    errors: list[dict] | None = Field(
        default=None, description="上传失败的文件列表（仅 status=partial 时出现）"
    )
...
class BasicInfo(BaseModel):
    ...
    confidentiality: str | None = Field(
        default=None, description="密级标识，如：株机公司普通商密▲5年"
    )
    document_type: str | None = Field(
        default=None, description="文件类型，如：工艺文件"
    )
    company: str | None = Field(
        default=None, description="公司名称，如：中车株洲电力机车有限公司"
    )
    manufacturing_center: str | None = Field(
        default=None, description="制造中心，如：城轨制造中心"
    )
    project_code: str | None = Field(
        default=None, description="项目号"
    )
    project_name: str | None = Field(
        default=None, description="项目名称/产品型号"
    )
    process_number: str | None = Field(
        default=None, description="工序号"
    )
    process_name: str | None = Field(
        default=None, description="工序名称"
    )
    document_number: str | None = Field(
        default=None, description="文件编号"
    )
    document_name: str | None = Field(
        default=None, description="文件名称"
    )
    part_drawing_number: str | None = Field(
        default=None, description="零部件图号"
    )
    part_name: str | None = Field(
        default=None, description="零部件名称"
    )
    version: str | None = Field(
        default=None, description="版本"
    )
    workstation: str | None = Field(
        default=None, description="工位"
    )
    effective_date: str | None = Field(
        default=None, description="日期"
    )
...
class ProcessStepResource(BaseModel):
    ...
    图片字段 image_id 需先通过 /api/process-card/images/upload 上传获取。
    """

    step_number: str = Field(description="工步号*")
    step_name: str = Field(description="工步名称*")
    component_drawing_number: str | None = Field(
        default=None, description="零部件图号（子图号）"
    )
    component_name: str | None = Field(
        default=None, description="零部件名称（子部件）"
    )
    step_marker: str | None = Field(
        default=None, description="工步标识（如：▲◆★ 分别代表不同重要等级）"
    )
    equipment_tooling: str | None = Field(
        default=None, description="设备、工艺装备（计量）"
    )
    ...
    image_id: str | None = Field(
        default=None,
        description="配图 — 先调用图片上传接口获取 image_id 后填入此处",
    )
...
class StepBody(BaseModel):
    ...
    所有 image_*_id 字段需先通过 /api/process-card/images/upload 上传获取。
    一个工步最多支持 1 张主配图 + 3 张附加配图页。
    """
    ...
    technical_requirements: str | None = Field(
        default=None, description="技术要求"
    )
    self_inspection: str | None = Field(
        default=None, description="是否自检（是/否）"
    )
    layout: str | None = Field(
        default=None, description="排版(横/竖)"
    )
    image_id: str | None = Field(
        default=None,
        description="配图（主图）— 先调用图片上传接口获取 image_id 后填入此处",
    )
    image_page_1_id: str | None = Field(
        default=None,
        description="配图页1 — 先调用图片上传接口获取 image_id 后填入此处",
    )
    image_page_2_id: str | None = Field(
        default=None,
        description="配图页2 — 先调用图片上传接口获取 image_id 后填入此处",
    )
    image_page_3_id: str | None = Field(
        default=None,
        description="配图页3 — 先调用图片上传接口获取 image_id 后填入此处",
    )
...
class ProcessCardInput(BaseModel):
    ...
    使用流程：
    1. POST /api/process-card/images/upload  上传全部图片，获取 image_id 列表
    2. POST /api/process-card/generate       提交本 JSON，图片字段填入对应的 image_id
    """
    ...
    normative_references: list[NormativeReference] = Field(
        default_factory=list, description="规范性引用文件"
    )
    material_list: list[MaterialItem] = Field(
        default_factory=list, description="物料清单"
    )
    process_step_resources: list[ProcessStepResource] = Field(
        default_factory=list, description="工步流程及作业资源需求"
    )
    step_bodies: list[StepBody] = Field(
        default_factory=list, description="工步正文"
    )
