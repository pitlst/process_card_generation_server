...
async function extractImagesFromXlsx(file: File): Promise<{
  ...
  const imageEntries = Object.keys(zip.files).filter((n) =>
    /^xl\/media\/image\d*\.(png|jpe?g|gif|bmp|webp)$/i.test(n)
  );
  ...
  for (const [sheetIdx, drawingPath] of drawingRels) {
    ...
    if (drRelsXml) {
      ...
      doc.querySelectorAll("Relationship").forEach((el) => {
        ...
        const target = el.getAttribute("Target");
        if (id && target)
          embedToFile.set(id, "xl/media/" + target.replace(/^.*\//, ""));
      });
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      resolve({
        ...
        normativeReferences: sheet("规范性引用文件") as Record<
          string,
          string | number
        >[],
        materialList: sheet("物料清单") as Record<
          string,
          string | number | null
        >[],
        processStepResources: sheet("工步流程及作业资源需求") as Record<
          string,
          string | number | null
        >[],
        stepBodies: sheet("工步正文") as Record<
          string,
          string | number | null
        >[],
      });
...
const BASIC_FIELD_MAP: Record<string, string> = {
  密级标识: "confidentiality", 文件类型: "document_type", 公司: "company",
  制造中心: "manufacturing_center", 项目号: "project_code", "项目名称/产品型号": "project_name",
  工序号: "process_number", 工序名称: "process_name", 文件编号: "document_number",
  文件名称: "document_name", 零部件图号: "part_drawing_number", 零部件名称: "part_name",
  版本: "version", 工位: "workstation", 日期: "effective_date",
  编制: "prepared_by", 校对: "checked_by", 审核: "reviewed_by",
  标准化: "standardized_by", 会签: "countersigned_by", 批准: "approved_by",
};

const CHANGE_MAP: Record<string, string> = {
  版本号: "version", 实施日期: "effective_date", 编制者: "author", 变更记录: "description", 文件状态: "status",
};

const REF_MAP: Record<string, string> = {
  序号: "sequence", 作业指导书编号: "document_number", 作业指导书名称: "document_name",
};

const MATERIAL_MAP: Record<string, string> = {
  "车种/分组": "vehicle_group", 序号: "sequence", 物料编码: "material_code",
  物料名称: "material_name", 规格型号: "specification", 数量: "quantity", 备注: "remark",
};

const RESOURCE_MAP: Record<string, string> = {
  "工步号*": "step_number", "工步名称*": "step_name",
  "零部件图号（子图号）": "component_drawing_number",
  "零部件名称（子部件）": "component_name", 工步标识: "step_marker",
  "设备、工艺装备（计量）": "equipment_tooling", 规格型号: "equipment_spec",
  作业工种: "work_type", 作业人数: "worker_count", "作业时间(min)": "work_time_minutes",
};

const STEP_BODY_MAP: Record<string, string> = {
  工步号: "step_number", 工步名称: "step_name", 动作序号: "action_sequence",
  动作描述: "action_description", 技术要求: "technical_requirements",
  是否自检: "self_inspection", "排版(横/竖)": "layout",
};

function remap<T extends Record<string, unknown>>(
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (uploadJson.status === "error")
        throw new Error(`图片上传失败: ${JSON.stringify(uploadJson.errors)}`);
      addLog(
        `图片上传完成 — ${uploadJson.total_count} 张成功` +
          (uploadJson.errors?.length ? `, ${uploadJson.errors.length} 张失败` : "")
      );

      const zipToId = new Map<string, string>();
      uploadJson.images.forEach(
        (img: { image_id: string; original_filename: string }, i: number) => {
          zipToId.set(cellPaths[i], img.image_id);
        }
      );
      ...
      for (const [cn, en] of Object.entries(BASIC_FIELD_MAP))
        basic[en] = preview.basicInfo[cn] || null;
      ...
      for (const r of changeRecords) {
        (r as Record<string, unknown>)["effective_date"] = excelDateToString(
          (r as Record<string, unknown>)["effective_date"]
        );
      }
      ...
      const resources = remap(preview.processStepResources, RESOURCE_MAP).map(
        (r: Record<string, unknown>, i: number) => {
          r["step_number"] = toStr(r["step_number"]);
          r["worker_count"] = toInt(r["worker_count"]);
          r["work_time_minutes"] = toInt(r["work_time_minutes"]);
          const matched = imgCells.find(
            (c) => c.sheetIndex === 4 && c.row === i + 2 && c.col === 10
          );
          return { ...r, image_id: matched ? zipToId.get(matched.zipPath) || null : null };
        }
      );
      apiJson["process_step_resources"] = resources;

      const bodyColMap: Record<number, string> = {
        7: "image_id", 8: "image_page_1_id", 9: "image_page_2_id", 10: "image_page_3_id",
      };
      const bodies = remap(preview.stepBodies, STEP_BODY_MAP).map(
        (b: Record<string, unknown>, i: number) => {
          ...
          for (const c of imgCells) {
            if (c.sheetIndex === 5 && c.row === i + 2) {
              ...
              if (field) imgFields[field] = zipToId.get(c.zipPath) || null;
            }
          }
          return { ...b, ...imgFields };
        }
    ...
    <div className="min-h-screen bg-neutral-50 p-6 dark:bg-neutral-950">
      <div className="mx-auto max-w-4xl space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-neutral-500 mt-1">
              上传 xlsx → 自动解析数据 & 提取图片 → 调用后端 API → 下载 PDF
            </p>
          </div>
          <button
        ...
        <div className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
          ...
          <input
            className="w-full rounded-md border px-3 py-1.5 text-sm font-mono bg-white text-black dark:bg-neutral-900 dark:text-white"
            value={apiBase}
        ...
        <div className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
          ...
          />
          {file && (
            <p className="mt-1 text-xs text-neutral-400">已选择: {file.name}</p>
          )}
        </div>
        ...
        {preview && (
          <div className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
            <h2 className="text-sm font-semibold mb-2 text-black dark:text-white">数据预览</h2>
            <div className="grid grid-cols-2 gap-3 text-xs">
              {[
                ...
                <div key={label}>
                  <span className="inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-semibold font-mono">
                    {label}
                  </span>
                  <pre className="font-mono text-xs break-all mt-1 max-h-32 overflow-auto text-neutral-700 dark:text-neutral-300">
                    {content}
                  </pre>
                </div>
              ))}
        ...
        {/* 日志 */}
        <div className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
          <h2 className="text-sm font-semibold mb-2 text-black dark:text-white">处理日志</h2>
          <pre
            ref={logRef}
            className="font-mono text-xs break-all max-h-64 overflow-auto whitespace-pre-wrap rounded-md bg-neutral-100 p-3 dark:bg-neutral-900 text-black dark:text-white"
          >
